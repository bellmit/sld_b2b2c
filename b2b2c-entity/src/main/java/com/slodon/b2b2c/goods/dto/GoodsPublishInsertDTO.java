...
public class GoodsPublishInsertDTO extends GoodsPublishFrontParamDTO {
    private static final long serialVersionUID = 3380236608629722937L;
    ...
    public GoodsPublishInsertDTO(GoodsPublishFrontParamDTO paramDTO) {
        ...
        //处理运费模板
        if (this.getFreightFee() != null){
            //设置了统一运费，运费模板置为0
            this.setFreightId(0);
        }
        //构造货品列表
        if (CollectionUtils.isEmpty(this.getSpecInfoList())){
            AssertUtil.notNullOrZero(this.getProductPrice(),"请填写商品价格");
            AssertUtil.notNullOrZero(this.getProductStock(),"请填写商品库存");
            AssertUtil.isTrue(this.getProductPrice().compareTo(new BigDecimal(9999999)) > 0
                    || this.getProductPrice().compareTo(new BigDecimal("0.01")) < 0,"销售价必须在0.01~9999999之间");
            if (!StringUtil.isNullOrZero(this.getMarketPrice())) {
                AssertUtil.isTrue(this.getMarketPrice().compareTo(new BigDecimal(9999999)) > 0
                        || this.getMarketPrice().compareTo(new BigDecimal("0.01")) < 0, "市场价必须在0.01~9999999之间");
                AssertUtil.isTrue(this.getMarketPrice().compareTo(this.getProductPrice()) <= 0, "销售价必须低于市场价");
            }
            //无规格，构造一个货品信息
            ProductInfo productInfo = new ProductInfo();
            productInfo.setMarketPrice(this.getMarketPrice());
            productInfo.setProductPrice(this.getProductPrice());
            ...
            List<ProductInfo> productInsertInfoList = new ArrayList<>();//入库货品信息列表
            for (int i = 0; i < this.getProductList().size(); i++) {
                ProductInfo productInfo = this.getProductList().get(i);
                AssertUtil.notNullOrZero(productInfo.getProductPrice(), "请填写第" + (i+1) + "个货品的价格");
                AssertUtil.notNullOrZero(productInfo.getProductStock(), "请填写第" + (i+1) + "个货品的库存");
                AssertUtil.isTrue(productInfo.getProductPrice().compareTo(new BigDecimal(9999999)) > 0
                        || productInfo.getProductPrice().compareTo(new BigDecimal("0.01")) < 0,"销售价必须在0.01~9999999之间");
                if (!StringUtil.isNullOrZero(productInfo.getMarketPrice())) {
                    AssertUtil.isTrue(productInfo.getMarketPrice().compareTo(new BigDecimal(9999999)) > 0
                            || productInfo.getMarketPrice().compareTo(new BigDecimal("0.01")) < 0, "市场价必须在0.01~9999999之间");
                    AssertUtil.isTrue(productInfo.getMarketPrice().compareTo(productInfo.getProductPrice()) <= 0, "销售价必须低于市场价");
                }
    ...
    public ProductInsertInfo getDefaultProduct() {
        for (ProductInfo productInfo : this.getProductList()) {
     ...
     * @return
     */
    public String getGoodsLabelIds(){
        if (CollectionUtils.isEmpty(this.getGoodsLabelList())){
            return null;
        }
        StringBuilder labelIds = new StringBuilder();
        this.getGoodsLabelList().forEach(goodsLabelInfo -> {
            labelIds.append(",").append(goodsLabelInfo.getGoodsLabelId());
        });
        return labelIds.toString().substring(1);
    }
    ...
    @EqualsAndHashCode(callSuper = true)
    @Data
    public static class ProductInsertInfo extends ProductInfo {
        private static final long serialVersionUID = 3169449502168640313L;

        public ProductInsertInfo(ProductInfo productInfo,List<SpecInfo> specInfoList,List<ImageInfo>  imageList) {
            //复制属性
         ...
         * @return
         */
        private String getProductMainImage(List<ProductInfo.ProductSpecInfo> productSpecInfoList,List<SpecInfo> specInfoList,List<ImageInfo>  imageList){
         ...
         * @return
         */
        private String getProductSpecValues(List<ProductInfo.ProductSpecInfo> specInfoList){
         ...
         * @return
         */
        private String getProductSpecValueIds(List<ProductInfo.ProductSpecInfo> specInfoList){
            if (CollectionUtils.isEmpty(specInfoList)) return "";
